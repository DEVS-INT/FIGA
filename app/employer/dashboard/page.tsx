...
const statusOptions: JobStatus[] = ["PENDING", "APPROVED", "REJECTED", "COMPLETED", "CANCELLED"];
...
export default function EmployerDashboardPage() {
  ...
  const handleJobAction = async (jobId: number, action: 'complete' | 'cancel' | 'reopen') => {
    try {
      ...
      switch (action) {
        ...
        case 'reopen': newStatus = 'APPROVED'; break;
      ...
      toast.success(`Job marked as ${newStatus.toLowerCase()}`, { id: "update-job" });
    } catch (error) {
            ...
            <CardHeader>
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  ...
                  <CardDescription>Manage and track all your jobs</CardDescription>
                </div>
                ...
                <TabsList className="grid w-full grid-cols-6">
                  ...
                  {statusOptions.map((status) => (
                    <TabsTrigger key={status} value={status}>
                      {status.charAt(0) + status.slice(1).toLowerCase()} ({getJobCountByStatus(status)})
                    </TabsTrigger>
                  ))}
                </TabsList>
                              ...
                              <div className="flex-1">
                                <div className="flex items-start justify-between mb-3">
                                  <div>
                                    <div className="flex items-center gap-2 mb-1">
                                      ...
                                      {job.job_urgency && (
                                        <Badge className={`${getUrgencyColor(job.job_urgency)} text-xs`}>
                                          {job.job_urgency}
                                        </Badge>
                                      )}
                                    </div>
                                  ...
                                  <Badge className={`${getStatusColor(job.status)} border`}>
                                    {getStatusIcon(job.status)} {job.status.charAt(0) + job.status.slice(1).toLowerCase()}
                                  </Badge>
                                </div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                  <div>
                                    ...
                                    <p className="text-sm font-medium">{job.shift_type}</p>
                                  </div>
                                  <div>
                                      ...
                                      <div className="flex items-center gap-2">
                                        <div className="h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center">
                                          <span className="text-xs font-medium">
                                            {job.assigned_caregiver.name.split(' ').map(n => n[0]).join('')}
                                          </span>
                                        </div>
                                        <div>
                                          <p className="text-sm text-blue-800 font-medium">
                                            Assigned: {job.assigned_caregiver.name}
                                          </p>
